...
getCar(new Car('toyota', 'white', 'japan')) // toyota is my car from japan in white color
 ...
 //-- or we can write this code in this way 

{
    ...
    const isAudi = (car : Car) : car is Audi => {
        return car instanceof Audi; 
    }
    const isMercedez = (car : Car) : car is Mercedez => {
        return car instanceof Mercedez;
    }

    const getCar = (car : Car) => {
        if(isAudi(car)){
            car.featureForAudi();
        }else if(isMercedez(car)){
            car.featureForMercedez();
        }else{
            car.myCar();
        }
